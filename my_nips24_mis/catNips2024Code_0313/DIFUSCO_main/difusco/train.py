...
def arg_parser():
  ...
  ## my 
  parser.add_argument('--my_limit_train_batches',type=int,default=None)
  parser.add_argument('--my_val_check_interval',type=int,default=None)
  parser.add_argument('--my_save_top',type=int,default=3)
  parser.add_argument('--my_data_path',type=str,default=None)
  parser.add_argument('--my_result_path', type=str, required=True)
  parser.add_argument('--my_training_seed',type=int,default=1234)
  parser.add_argument('--tmp_data_path_r',type=float,default=-1)
  parser.add_argument('--tmp_data_path_kk',type=int,default=-1)
  parser.add_argument('--my_coreset_size',type=int,default=-1)
...
def main(args):
  ...
  wandb_logger = WandbLogger(
      ...
      save_dir=os.path.join(args.my_result_path, f'models'),
      id=args.resume_id or wandb_id,     
  )
  
  
  #wandb_logger.experiment.config.update( {"kk":args.tmp_data_path_kk,"r":args.tmp_data_path_r} )
  ...
  checkpoint_callback = ModelCheckpoint(
      ...
      save_top_k=args.my_save_top, save_last=True,
  ...
  ## my 
  my_limit_train_batches = args.my_limit_train_batches
  my_val_check_interval = args.my_val_check_interval

  trainer = Trainer(
      limit_train_batches = my_limit_train_batches,
      accelerator="auto",
      devices=torch.cuda.device_count() if torch.cuda.is_available() else None,
      max_epochs=epochs,
      callbacks=[TQDMProgressBar(refresh_rate=20), checkpoint_callback, lr_callback],
      logger=wandb_logger,
      val_check_interval=my_val_check_interval,
  ...
  if args.do_train:
    ...
    print("ckpt_path for test = ",ckpt_path)
    trainer.validate(model, ckpt_path=ckpt_path)
    if not args.do_valid_only:
      trainer.test(model, ckpt_path=ckpt_path)
  trainer.log("two_opt_iterations",args.two_opt_iterations,"batch_size",args.batch_size,"kk",args.tmp_data_path_kk,"r",args.tmp_data_path_r,"sample_size",args.my_coreset_size)
