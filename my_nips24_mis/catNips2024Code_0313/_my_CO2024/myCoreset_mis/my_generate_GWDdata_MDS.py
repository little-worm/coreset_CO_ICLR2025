...
import numpy as np
from catNips2024Code_0313._my_CO2024.myCoreset_mis.mytreePackage.myTree_mis_MDS import my_RWD_coreset_MDS




def my_generate_RWDdata_MDS(org_filepath,org_filename,kk,maxPoolNum,ballRadius_RWD=0.052,my_embeding_dim=3):
    ...
    coreset_misFilename_list = my_RWD_coreset_MDS(global_misFilename_list,ballRadius_RWD,kk,maxPoolNum,my_embeding_dim=my_embeding_dim)
    # my_RWD_coreset(global_misFilename_list,ballRadius_RWD=0.1,kk=4,maxPoolNum=40,my_embeding_dim=3,RWD_iter=5)

    mySavePath = org_filepath + '/mds/res_' + org_filename + str(len(global_misFilename_list))+  '_r' + str(ballRadius_RWD) + '_kk' + str(kk)+ '_pool' + str(maxPoolNum) + '_' + str(len(list(coreset_misFilename_list))) 
...
def test_100(kk=2):
    for ballRadius_RWD in [13.06,12.7]:
        ...
        kk=4;maxPoolNum=96*2
        my_generate_RWDdata_MDS(org_filepath,org_filename,kk,maxPoolNum,ballRadius_RWD,my_embeding_dim=3)
