...
def my_Graph_embeding_spring(arg):
    misFilename,my_embeding_dim,MAX_NUM_NODES = arg
    G = nx.read_gpickle(misFilename)
    num_nodes = G.number_of_nodes()
    sub_num_nodes = 100
    pos = nx.spring_layout(G, dim=my_embeding_dim)  # 快速计算嵌入
    pos = list(pos.values())
...
def my_RWD_coreset_spring(global_misFilename_list,ballRadius_RWD=0.1,kk=4,maxPoolNum=96,my_embeding_dim=3,RWD_iter=5,MAX_NUM_NODES=1000):
    ...
    #     if cou %1000 == 0:
    #         print("------------------ cou = ",cou)
    #     res = my_Graph_embeding_spring(arg)
    #     tmp_res.append(res)
    ...
    with Pool(maxPoolNum) as pool:
        tmp_res_0 = pool.map(my_Graph_embeding_spring,tmp_arg_list[:125000])

    with Pool(maxPoolNum) as pool:
        tmp_res_1 = pool.map(my_Graph_embeding_spring,tmp_arg_list[125000:])

    tmp_res = list(tmp_res_0) + list(tmp_res_1)  
        
    # with Pool(maxPoolNum) as pool:
    #     tmp_res = pool.map(my_Graph_embeding_spring,tmp_arg_list)
...
def test_my_RWD_coreset():
    ...
    my_RWD_coreset_spring(global_misFilename_list,ballRadius_RWD=0.1,kk=4,maxPoolNum=40,my_embeding_dim=3,RWD_iter=5)
    print("")
