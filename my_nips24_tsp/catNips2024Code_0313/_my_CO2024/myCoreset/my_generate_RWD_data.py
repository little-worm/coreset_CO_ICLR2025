...
def my_generate_RWDdata(org_filepath,org_filename,n,m,ballRadius,kk,maxPoolNum,point_dim,my_maxIterTimes_RWD=5):
    ...
    org_coreset_locations_lists,new_coreset_locations_lists,coreset_tours_lists,my_tree = my_RWD_coreset(org_location_lists,org_tour_lists,ballRadius,kk,maxPoolNum,maxIterTimes_RWD=my_maxIterTimes_RWD)
    ...
    org_tspDataCoresetFilename_RWD = mySavePath + '/org/' +  org_filename + "_myRWDCoreset"
    new_tspDataCoresetFilename_RWD = mySavePath + '/new/' +  org_filename + "_myRWDCoreset"

    my_save_tspData(org_tspDataCoresetFilename_RWD,org_coreset_locations_lists,coreset_tours_lists,point_dim)      
    my_save_tspData(new_tspDataCoresetFilename_RWD,new_coreset_locations_lists,coreset_tours_lists,point_dim)      

    #ids_weights_Filename = mySavePath + '/' + 'id_weights'

    #np.save(ids_weights_Filename, [[child_id_flatenList],[child_weight_flatenList]])
  
    org_mySaveTreeFilename = mySavePath + '/org/' + 'tree.plk'
    new_mySaveTreeFilename = mySavePath + '/new/' + 'tree.plk'

    with open(org_mySaveTreeFilename, 'wb') as file:
        pickle.dump(my_tree, file)
    with open(new_mySaveTreeFilename, 'wb') as file:
        pickle.dump(my_tree, file)        
...
def test_100(kk=2):
    for ballRadius in [0.9,0.945,1.004]:
        print("-----------------------")
        n = 128000; m = 100; 
